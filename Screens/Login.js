...
const Login = ({ navigation }) => {
  const [userpass, setLogin] = useState({ password: '', name: '' });
  const [visible, setVisible] = useState(false)
  // const [username,setUname] = useState('');
  // const [password, setPass] = useState('');
  const onLogin = () => {
    if (userpass.name == 'harshil' && userpass.password == 'h12') {
      navigation.navigate('Dashboard', { name: userpass.name });
    } else {
      Alert.alert('error');
    }
  };
  ...
  return (
    <LinearGradient colors={['#3D7EAA', '#E4E5E6']} style={styles.container}>
      <View style={{ flex: 1, alignItems: 'flex-start', margin: 5, position: "absolute" }}>
      ...
      <View style={{ flex: 7, alignItems: 'center' }}>
        ...
        <TextInput
          ...
          onChangeText={text => {
            setLogin({ password: userpass.password, name: text });
          }}
        />
        <TextInput
          style={styles.textView}
          ...
          secureTextEntry
          onChangeText={text => {
            setLogin({ password: text, name: userpass.name });
          }}
        />
        <View
          style={{
